...
SoundBank::SoundBank(std::shared_ptr<Rom> rom) : rom(std::move(rom)) {}
...
Sequence::Sequence(std::shared_ptr<Rom> rom, uint8_t trackLimit)
    : memaccArea(256), rom(std::move(rom)), trackLimit(trackLimit)
{
    Init(0);
}
...
uint8_t Sequence::GetReverb() const
{
    ...
    return rom->ReadU8(songHeaderPos + 3);
}
...
uint8_t Sequence::GetPriority() const
{
    ...
    return rom->ReadU8(songHeaderPos + 2);
}
...
SongTable::SongTable(std::shared_ptr<Rom> rom, size_t songTablePos)
    : rom(std::move(rom)), songTablePos(songTablePos)
{
...
size_t SongTable::GetPosOfSong(uint16_t uid) {
    return rom->ReadAgbPtrToPos(songTablePos + uid * 8);
}
